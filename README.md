...
- 月別のデータの場合、"prefix_excel.py"ファイルのメインセクションの１行目に都道府県名をクラスとしてセットし、実行します（このファイルはmainとして実行されます）。指示に従ってファイルを入力してください（複数のファイルを選択することができます）。スクリプトファイルの名称にかかわらず、データはエクセルファイルでもcsvファイルでも問題ありません。  
